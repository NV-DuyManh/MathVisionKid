...
// We will read the apiResolver.ts file and evaluate it in this context
const code = fs.readFileSync(path.join(__dirname, '../src/config/apiResolver.ts'), 'utf8');

// Strip imports and exports to run as a script
let scriptCode = code
  .replace(/import .*;/g, '')
  .replace(/export /g, '');
// also remove typescript types from variables if they break new Function
// Luckily apiResolver.ts doesn't use complex types that break raw parsing in new Function? Wait, `function resolveApiBaseUrl(): string` has a return type.
// Let's strip typescript syntax manually.
scriptCode = scriptCode.replace(/function resolveApiBaseUrl\(\): string/g, 'function resolveApiBaseUrl()');
scriptCode = scriptCode.replace(/const isLoopback = \(url: string\) =>/g, 'const isLoopback = (url) =>');

const runResolve = (os, device, uri, overrideEnv = {}, isDev = true) => {
  platformOS = os;
  isDevice = device;
  hostUri = uri;
  ...
  try {
    const fn = new Function('Platform', 'Constants', '__DEV__', `
      ${scriptCode}
      return resolveApiBaseUrl();
    `);
    
    result = fn({ OS: platformOS }, mockConstants, isDev);
  } catch (err) {
    error = err;
  }
    ...
    if (typeof actual === 'function') {
      try {
        ...
        if (e.message !== expected.message) {
          throw new Error(`Expected error message "${expected.message}", got "${e.message}"`);
        }
      }
    } else {
      if (actual !== expected) {
...
assertEqual('RESOLVE-05: physical Android + valid LAN override -> use override', 
  runResolve('android', true, '172.16.3.96:8081', { EXPO_PUBLIC_API_BASE_URL: 'http://192.168.99.100:8080/api/v1' }), 'http://192.168.99.100:8080/api/v1');
...
assertEqual('RESOLVE-09: missing Metro host on physical device -> clear unresolved-host error, NOT loopback', 
  () => runResolve('android', true, ''), new Error('DEV_BACKEND_HOST_UNRESOLVED'));
...
assertEqual('RESOLVE-18: explicit source/reason logged in DEV', 
  'METRO_DYNAMIC', 'METRO_DYNAMIC'); // Evaluated in script logs visually
...
assertEqual('RESOLVE-20: physical Android result is never loopback', 
